	...
	if [ "$process" = "1" ]
		...
		do
			...
			if [ "$printpdfmathjax" = "" ]; then
				...
				if [ "$printpdfsubdirectory" = "" ]; then
					gulp mathjax --book $bookfolder --output printpdf
				else
					gulp mathjax --book $bookfolder --language $printpdfsubdirectory --output printpdf
				fi
			fi

			echo "Creating targets for book index references..."
			if [ "$printpdfsubdirectory" = "" ]; then
				gulp renderIndexCommentsAsTargets --book $bookfolder --output printpdf
			else
				gulp renderIndexCommentsAsTargets --book $bookfolder --language $printpdfsubdirectory --output printpdf
			fi
	...
	##############
		...
		do
			...
			if [ "$screenpdfmathjax" = "" ]; then
				...
				echo "Processing MathJax in HTML."
				if [ "$screenpdfsubdirectory" = "" ]; then
					gulp mathjax --book $bookfolder --output screenpdf
				else
					gulp mathjax --book $bookfolder --language $screenpdfsubdirectory --output screenpdf
				fi
			fi

			echo "Creating targets for book index references..."
			if [ "$screenpdfsubdirectory" = "" ]; then
				gulp renderIndexCommentsAsTargets --book $bookfolder --output screenpdf
			else
				gulp renderIndexCommentsAsTargets --book $bookfolder --language $screenpdfsubdirectory --output screenpdf
			fi

...
Our design includes styling for a range of classes. You can apply these to elements in markdown.

Use these classes in your markdown to create specific formatting effects. A class that is applicable to blocks of text can be implemented by placing the class in curly brackets, after a colon and a dot, for example ```{:.box}``` should be applied on the line directly below the box text.

You can also apply more than one class to an element within one set of curly brackets. For example:

```This is a paragraph which will turn into a wide box.
{:.box .wide}```
...
In addition to the classes that the Electric Book template ships with (listed below), the CORE project includes several new ones:

* `.info`: ancillary info in a box in the sidebar
* `.exercise`: an exercise
* `.great-economists`: a box of ancillary info in the main text area, used for 'Great economist' boxes
* `.economists`: a box of ancillary info in the main text area, used for 'When economists disagree' and 'How economists learn from facts' boxes
* `.einstein`: a box of ancillary info in the main text area

(These features do not also require `.sidenote` or `.box` classes as well.)

## Default Electric Book classes

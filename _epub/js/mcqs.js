...
var ebMCQsGetAllSelected = function(mcqsToCheck) {
    ...
    // set it all false for now
    var allTheOptions = mcqsToCheck.querySelectorAll(".mcq-options li");
    allTheOptions.forEach(function(selectedAnswer, index){
        selectedOptions[index + 1] = false;
    });

    // update for the selected ones
    var selectedAnswers = mcqsToCheck.querySelectorAll(".selected");
    selectedAnswers.forEach(function(selectedAnswer) {
        var dataIndex = parseFloat(selectedAnswer.getAttribute('data-index'));

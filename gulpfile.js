...
// Do we want cheerio to run in xmlParser mode during indexing tasks?
var xmlParser = true;
if (output === "printpdf" || output === "screenpdf") {
    xmlParser = false;
}
...
var paths = {
    ...
    js: {
        src: [
            ...
            'assets/js/expandable-box.js',
            'assets/js/sidenotes.js',
            'assets/js/annotation.js',
            'assets/js/wordpress-user-profile.js'
        ],
        dest: 'assets/js/'
    }
};
...
// All internal links and IDs need to have non-ascii characters
// removed so that internal links work in Apple Books

gulp.task('epub:xhtmlLinks', function (done) {
    'use strict';

    gulp.src([paths.epub.src, '_site/epub/package.opf', '_site/epub/toc.ncx'], {base: './', allowEmpty: true})
        .pipe(cheerio({
            run: function ($) {
            ...
            run: function ($) {
                ...
                $('*').contents()
                    ...
                    .each(function (index, comment) {
                        ...
                        // create a wrapper in which to house the block comment
                        var newBlockCommentSpan = $("<span></span>")
                            .addClass("index-comment-span")
                            .addClass("keep-together")
                            .addClass("keep-with-next");
                        ...
                        commentLines.forEach(function (line) {
                            ...
                            indexTerm = true;
                            var entrySlug = ebSlugify(line, indexTerm);
                            ...
                            // We don't want to use a wrapper for epubs, since
                            // this violates EpubCheck rules in some instances
                            if (position === "block" &&
                                (output == "screenpdf" || output == "printpdf")
                            ) {
                                newAnchorElement.appendTo(newBlockCommentSpan);
                            } else {
                                newAnchorElement.insertAfter(comment);
                            }
                        });
                        // If we have a block comment div, we need to pop it into
                        // the DOM now so that we don't lose it
                        if (position === "block" &&
                            (output == "screenpdf" || output == "printpdf")
                        ) {
                            newBlockCommentSpan.insertAfter(comment);
                        }
                    });
                ...
                $('.index-comment-span').each(function (i, commentSpan) {

                    var commentSpan = $(commentSpan); // wrap it for cheerio
                    var indexedElement = $(commentSpan).nextAll(':not(.index-target)').first();
                    indexedElement.prepend(commentSpan);
                });
        ...
        .pipe(cheerio({
            run: function ($) {
                ...
                function ebIndexFindLinks(listItem, ebIndexTargets) {
                    ...
                    // and save its slug.
                    indexTerm = true;
                    var listItemSlug = ebSlugify(listItemTree.join(' \\\\ '), indexTerm);
                    ...
                    // Links to index points in the book are numbered sequentially

...
var ebSlidesBuildNav = function(slidelines) {
    slidelines.forEach(function(slideline){
        ...
        var slidelineID = slideline.getAttribute("id");
...
var ebSlidesShow = function(slidelines) {
    ...
    slidelines.forEach(function(slideline) {
        ...
        if (!slideline.querySelector(sanitisedTargetHash)) {
            ...
            var element = document.querySelector(sanitisedTargetHash);
            var ancestor = element.closest("div.figure");
...
var ebSlidesKeyDown = function() {
    ...
    // Figures that are not visible should not be tabindexed
    var slideDivs = document.querySelectorAll(".slides");

    slideDivs.forEach(function(slideDiv) {
        var slideThumbnails = slideDiv.querySelectorAll(".nav-slides li a");
        var slideFigures = slideDiv.querySelectorAll(".figure a.figure-image-link");
        if (slideFigures.length !== 0) {
            slideThumbnails.forEach(function(thumbnail) {
                var id = thumbnail.href.split("#")[1];
                var figure = slideDiv.querySelector("#" + id + " a");
                // Leave the first one indexed to begin
                if (slideThumbnails.item(0) !== thumbnail) {
                    figure.setAttribute("tabindex", "-1");
                }
                thumbnail.addEventListener("keydown", function(event) {
                    if (event.key === "Enter") {
                        // add tabindex to all of them again
                        slideFigures.forEach(function(fig) {
                            fig.setAttribute("tabindex", "-1");
                        });
                        // and then remove from the visible one

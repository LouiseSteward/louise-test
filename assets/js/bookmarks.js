...
function ebBookmarksToggleModal(modal) {
    ...
    ebToggleClickout(modal, function () {
        ...
        // Otherwise, show it
            ...
            let loginLink = modal.querySelector('a.bookmark-dialog-login');
            ...
            if (loginLink) {
                if (ebWordpressIsLoggedIn()) {
                    ...
                    let redirectUrl = `${window.location.origin}/login?redirect_to=${window.location.href}`;
...
function ebPrepareForBookmarks() {
    ...
    // Manully trigger mutation observer
    document.body.setAttribute('data-bookmarks-initialised', true);
}
